...
function sampleTextureNearest(texture, x, y) {
...
function sampleTextureNearestByUv(texture, u, v) {
...
function findInterpolatingFactorForNearPlane(p1, p2) {
...
function clipTriangleAgainstNearPlane(p1, p2, p3) {
...
function DrawMesh(mesh, transform, cam) {
    ...
    const projectedPointsBeforDevide = pointsInCameraSpace.map((point, index) => perspectiveProjectionBeforePerspectiveDevide({
        pos: point,
    ...
    mesh.triangleIndicesData.forEach(([a, b, c]) => {
        ...
        let col = { r: shade, g: shade, b: shade, a: 255 };
        col = {
...
function RasteriseTriangle(p1, p2, p3, col, shadeMultiplier = 1, texture) {
